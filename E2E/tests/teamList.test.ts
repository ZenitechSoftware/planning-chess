...
Scenario('Players who skipped their moves are at the bottom of the list', async () => {
...
Scenario('Current voter is at the top of the list while game is in progress', async () => {
    login.firstVoterLogin(username.user1);
    let url = await I.grabCurrentUrl();
    session(username.user2, () => {
        login.voterLoginIntoCreatedGameRoom(url, username.user2);
    });
    I.seeTextEquals(`${username.user1} (you)`, game.locator.playersList.playerUsernameByIndex(1));
    session(username.user2, () => {
        I.seeTextEquals(`${username.user2} (you)`, game.locator.playersList.playerUsernameByIndex(1));
    });
});

Scenario('Players are listed based on individual final score in descending order', async () => {
    login.firstVoterLogin(username.user1);
    let url = await I.grabCurrentUrl();
    session(username.user2, () => {
        login.voterLoginIntoCreatedGameRoom(url, username.user2);
