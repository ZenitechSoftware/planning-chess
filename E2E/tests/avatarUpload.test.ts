...
Scenario("Player uploads avatar during login", async () => {
  ...
  await assertions.checkIfTheProfileImageIsUploaded(
    username.user1,
...
Scenario("Spectator uploads avatar during login", async () => {
  login.spectatorUploadAvatarDuringLogin(
    username.user1,
    imageLink.birdImageLink
  );
  const avatarImageTopRightCorner = await I.grabAttributeFrom(
...
Scenario(
  "Invalid image link is entered in avatar link field during login",
  () => {
    login.voterUploadAvatarDuringLogin(
      username.user1,
      imageLink.invalidImageLink
    );
...
Scenario(
  "Invalid image link is entered in avatar link field in the game room",
  () => {
    login.firstVoterLogin(username.user1);
...
Scenario(
  "Player who made a move on the chessboard changes his profile image",
  async () => {
    login.firstVoterLogin(username.user1);
    ...
    session(username.user2, async () => {
      const firstPlayersImageOnThePlayersList = await I.grabAttributeFrom(
        game.locator.playersList.avatarImageInThePlayersList(username.user1),
        "src"
      );
...
Scenario(
  "Users avatar does not disappear after leaving and returning back to the game room",
  async () => {
    login.voterUploadAvatarDuringLogin(username.user1, imageLink.dogImageLink);
    await assertions.checkIfTheProfileImageIsUploaded(
      username.user1,
